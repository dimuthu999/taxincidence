...
# sold_data <- readRDS(file="ca_anchoring_10.rds")
...
# 
# 
# sold_data['listed_var'] <- sold_data$listed_year
# sold_data$listed_var <- ifelse(sold_data$listed_var>2015,2015,sold_data$listed_var)
# sold_data['listed_var'] <- sapply(sold_data$listed_var,function(x) paste("proptax_",x,sep=""))
# 
# for(i in 1:nrow(sold_data)){
#   eval(parse(text=paste("sold_data[i,'listed_tax'] <- sold_data[i,'",sold_data[i,'listed_var'],"']",sep="")))
# }
# sold_data$listed_var <- NULL
# 
# 
...
# 
# purchased_price_estimate <- felm(purchased_amount~beds+baths+purchased_hpi|factor(zip_purchased_year),data=sold_data)
...
# other['purchase_price_residual'] <- NA
# 
# 
...
# 
# 
# other['tax_pct'] <- other$listed_tax/other$estimated_current_value
...
# sold_data['loss'] <- sold_data$purchased_amount-sold_data$estimated_current_value
# 
...
# other['loss'] <- other$purchased_amount-other$estimated_current_value
# 
# 
# sold_data <- rbind(sold_data,other)
# sold_data['log_loss'] <- ifelse(sold_data$loss<=0,0,log(sold_data$loss))
...
# saveRDS(sold_data,file = "sold_data_cleaned_10.rds")

sold_data <- readRDS(file = "sold_data_cleaned_10.rds")

# sold_data <- readRDS(file="ca_anchoring_10.rds")
# temp <- sold_data[sold_data$state %in% c("CA") & sold_data$purchased_year>1978,]
# 
...
stargazer(temp[, c("degree_of_overpricing","tax_pct","listing_amount","listed_year","current_value","sales_price","beds","baths","sqft","avg_school_rating","avg_school_distance","age","Lot","Lastremodelyear","purchased_amount","purchased_year","ownership_years")], type = output.type, summary.stat = c("mean", "sd", "p25", "median", "p75", "n"),notes = "")
```
...
temp <- ddply(temp,.(purchased_quarter),summarise,proptax_2015 = mean(proptax_2015),estimated_current_value=mean(estimated_current_value),taxpct = proptax_2015*100/estimated_current_value,hpi=mean(purchased_hpi,na.rm=TRUE))
...
temp <- sold_data[sold_data$state %in% c("CA") & sold_data$purchased_year<=2012 & sold_data$purchased_year>=2006,]
temp <- temp[temp$degree_of_overpricing>=quantile(temp$degree_of_overpricing,0.02,na.rm = TRUE) & temp$degree_of_overpricing<=quantile(temp$degree_of_overpricing,0.98,na.rm = TRUE) & temp$tax_pct>=quantile(temp$tax_pct,0.02,na.rm = TRUE) & temp$tax_pct<=quantile(temp$tax_pct,0.98,na.rm = TRUE),]
# estimated_list_pirce <- lm(listing_amount~listed_hpi+ownership_years+avg_school_rating+walk_score+Lot+baths+beds+age+factor(zip_listed_year)+factor(zip_purchased_year),data=temp)
# temp['estimated_list_pirce'] <- predict(estimated_list_pirce)
...
controls[[5]] <- "log(estimated_current_value)+purchase_price_residual+listed_hpi+Lot+beds+age+ownership_years+avg_school_rating+walk_score|zip_listed_year"
main_result <- list()
