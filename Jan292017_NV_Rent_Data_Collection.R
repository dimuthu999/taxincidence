...
zip_population <- read.csv(file = "zipcode_population_NV.csv")
zips <- zip_population[zip_population$population>30000 & !is.na(zip_population$population),]$zip
links <- list()
zip_no=0
j=1
for(zip in zips)  {
        ...
        for(page in 1:no_of_pages) {
          ...
          if(length(homedetails[[1]])>0)  {
            for(i in 1:length(homedetails[[1]]))  {
              ...
              # cat(links[[j]][1])
              write.table(link,file="NV_Rent_Links.csv",sep="|",append = TRUE,row.names = FALSE,col.names = FALSE)
              j=j+1
              #print(links[[i]])
            }
          }
