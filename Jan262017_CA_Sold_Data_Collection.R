...
current_url <- "http://www.zillow.com/homes/recently_sold/house_type/94112_rb/"
...
initializ_collection <- function()  {
  ...
  login$sendKeysToElement(list(key = "control", "-"))
  login$sendKeysToElement(list(key = "control", "-"))
  login$sendKeysToElement(list(key = "control", "-"))
  login$sendKeysToElement(list(key = "control", "-"))
  #login$sendKeysToElement(list("\uE00A","\uE00D","x"))
...
zips <- zip_population[zip_population$population>50000 & !is.na(zip_population$population),]$zip
links <- list()
zip_no=0
j=1
for(zip in zips)  {
  tryCatch({
    ...
    tryCatch({
      ...
      wxbox$sendKeysToElement(list(as.character(zip), "\uE007"))
        ...
        no_of_pages <- min((no_of_results %/% 26)+1,20)
        ...
        for(page in 1:no_of_pages) {
          ...
          if(length(homedetails[[1]])>0)  {
            for(i in 1:length(homedetails[[1]]))  {
              ...
              # cat(links[[j]][1])
              write.table(link,file="CA_Sold_Links.csv",sep="|",append = TRUE,row.names = FALSE,col.names = FALSE)
              j=j+1
              #print(links[[i]])
            }
          }
          ...
          if(page < no_of_pages) {
            withTimeout({
              mybrowser$navigate(paste("http://www.zillow.com/homes/recently_sold/house_type/",zip,"_rb/",(page+1),"_p/",sep = ""))
...
fn = "California_Sold_Data_3.csv"

rec_no = 31068
for(link in links[31068:length(links)]) {
  tryCatch({
    rec_no = rec_no+1
    ...
    # temp <- NULL
    # tryCatch({temp <- mybrowser$findElements(using='id','hdp-neighborhood')},error=function(cond) {})
    # if(length(temp)==1) {
    #   
    ...
    tryCatch({
      temp <- mybrowser$findElement(using='class','zsg-content-section ')
      temp$clickElement()
      #Sys.sleep(5)
      temp$sendKeysToElement(list(key = "up_arrow"))
      temp$sendKeysToElement(list(key = "up_arrow"))
    ...
    # pagesource <- mybrowser$getPageSource()
    # #<a id="yui_3_18_1_1_1485532554646_3170" class="zsg-tab-link" href="#hdp-tax-history">Tax History</a>
    # tax_id <- gregexpr('\" class=\"zsg-tab-link\"', pagesource)
    # substr(pagesource,tax_id[[1]][2]-67,tax_id[[1]][2]-37)
    # tryCatch({
    #   temp <- mybrowser$findElement(using = 'id',substr(pagesource,tax_id[[1]][2]-67,tax_id[[1]][2]-37))
    #   temp$clickElement()
    # },error=function(cond) {
    #   temp <- mybrowser$findElements(using='class','zsg-tab-link')
    #       tryCatch({
    #         temp[[2]]$clickElement()
    #       },error=function(cond) {})
    # })
    ...
    tryCatch({
      ...
      temp <- mybrowser$findElements(using='class','maximize')
      Sys.sleep(1)
      tryCatch({
        temp[[2]]$clickElement()
      },error=function(cond) {
