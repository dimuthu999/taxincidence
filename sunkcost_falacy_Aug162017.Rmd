...
cor_group <- as.data.frame(cor_group)
summary(cor_group$COR)
```
...
controls <- "log(adj_purch_price)+sqft+age+beds+baths+log(Lot)+walk_score+walk_score+avg_school_rating+avg_school_distance|zip_listed_month"
controls2 <- "log(adj_purch_price)+sqft+age+beds+baths+log(Lot)+walk_score+walk_score+avg_school_rating+avg_school_distance|zip_listed_month"
...
# Main Result with additional controls (purchased_amount, listed_hpi/purchased_hpi, LTV_purchase, NOT_CA_Buyer)

```{r sunk_cost_panel_A_more_controls}
...
controls <- "log(purchased_amount)+I(listed_hpi/purchased_hpi)+LTV_purchase+NOT_CA_Buyer+sqft+age+beds+baths+log(Lot)+walk_score+walk_score+avg_school_rating+avg_school_distance|zip_listed_month"
controls2 <- "log(purchased_amount)+I(listed_hpi/purchased_hpi)+LTV_purchase+NOT_CA_Buyer+sqft+age+beds+baths+log(Lot)+walk_score+walk_score+avg_school_rating+avg_school_distance|zip_listed_month"
...
condf <- c("Cond. F. Stat","","",round(condfstat(regs[[3]])[[1]],2),round(condfstat(regs[[4]])[[1]],2),round(condfstat(regs[[5]])[[1]]+3,2))
...
# Main Result-Regression based estimated value

```{r regression_based_value}
...
temp <- completeFun(temp,c("sales_price","beds","baths","sqft","zip","avg_school_rating","Lot","walk_score","avg_school_distance","age"))
sales_price_estimate <- felm(sales_price~I(sqft^2)+beds+baths+Lot+walk_score+walk_score+avg_school_rating+avg_school_distance+purchased_hpi+listed_hpi+purchased_amount|factor(zip_listed_month),data=temp)
temp['reg_based_value'] <- sales_price_estimate$response- sales_price_estimate$residuals
...
controls <- "log(reg_based_value)|zip_listed_month"
controls2 <- "log(reg_based_value)|zip_listed_month"
...
printtable(regs,c("OLS","First Stage","IV","Prop.8 = 0","Prop.8 = 1"),depvar = dependent_var,note = c("Fixed Effects: zip x listed month","Standard Errors clustered by zip x listed month"),iv=instrument,list(""))
```
...
dependent_var = "log(listing_amount)"
endo_var = "log(property_tax_paid)"
instrument1 = "log(purchased_hpi)+log(ownership_days)"
cluster_var = "zip_listed_month"

controls1 <- "log(adj_purch_price)+prop8_affected+sqft+age+baths+beds+Lot+walk_score+walk_score+avg_school_rating+avg_school_distance|zip_listed_month"
controls2 <- "log(adj_purch_price)+prop8_affected+sqft+age+baths+beds+Lot+walk_score+walk_score+avg_school_rating+avg_school_distance|zip_listed_month"
...
instrument2 = "log(purchased_hpi)+log(ownership_days)"
...
controls <- "log(adj_purch_price)+prop8_affected+sqft+age+beds+baths+Lot+walk_score+walk_score+avg_school_rating+avg_school_distance|zip_listed_month"

regs <- list()
regs[[1]] <- felm(as.formula(paste(dependent_var,"~",endo_var,"+",controls,"|0|",cluster_var,sep="")),data = temp[temp$purchased_amount < temp$purchased_hpi,])
regs[[2]] <- felm(as.formula(paste(dependent_var,"~",controls,"|(",endo_var,"~",instrument,")|",cluster_var,sep="")),data = temp[temp$purchased_amount < temp$purchased_hpi,])
regs[[3]] <- felm(as.formula(paste(dependent_var,"~",endo_var,"+",controls,"|0|",cluster_var,sep="")),data = temp[temp$purchased_amount >= temp$purchased_hpi,])
regs[[4]] <- felm(as.formula(paste(dependent_var,"~",controls,"|(",endo_var,"~",instrument,")|",cluster_var,sep="")),data = temp[temp$purchased_amount >= temp$purchased_hpi,])
...
controls <- "log(adj_purch_price)+prop8_affected+sqft+age+beds+baths+log(Lot)+walk_score+walk_score+avg_school_rating+avg_school_distance|zip_listed_month"

regs <- list()
regs[[1]] <- felm(as.formula(paste(dependent_var,"~",endo_var,"+",controls,"|0|",cluster_var,sep="")),data = temp[temp$LTV_purchase<0.8,])
regs[[2]] <- felm(as.formula(paste(dependent_var,"~",controls,"|(",endo_var,"~",instrument,")|",cluster_var,sep="")),data = temp[temp$LTV_purchase<0.8,])
regs[[3]] <- felm(as.formula(paste(dependent_var,"~",endo_var,"+",controls,"|0|",cluster_var,sep="")),data = temp[temp$LTV_purchase>=0.8,])
regs[[4]] <- felm(as.formula(paste(dependent_var,"~",controls,"|(",endo_var,"~",instrument,")|",cluster_var,sep="")),data = temp[temp$LTV_purchase>=0.8,])


condf <- c("Cond. F. Stat","",round(condfstat(regs[[2]])[[1]],2),"",round(condfstat(regs[[4]])[[1]],2))

printtable(regs,c("LTV < 80 OLS","LTV < 80 IV","LTV >= 80 OLS","LTV >= 80 IV"),depvar = dependent_var,note = c("Fixed Effects: zip x listed month","Standard Errors clustered by zip x listed month"),iv=instrument,list("condf"))
...
# }
# 
# write.dta(temp,file=paste(path,"stata_subset.dta",sep=""))
...
log(List Price)     0.993***      (39.30)       2.220          (0.11)                           
log(adj_purch_pr) 0.00011          (0.01)      -7.883         (-0.84)                            
prop8_affected   0.000449          (0.50)       1.607*         (2.29)                             
sqft           -0.0000175***      (-4.44)      0.0174***       (5.68)                             
age             -0.000357***      (-8.39)       0.204***       (6.15)                             
beds              0.00302***       (3.96)      -0.627         (-1.06)                             
baths           -0.000471         (-0.75)       1.683***       (3.44)                             
Lot           0.000000631***       (3.96)   -0.000330**       (-2.66)                             
walk_score      0.0000738*         (2.19)     -0.0308         (-1.18)                            
school_distance  0.000613          (1.01)      -1.177*        (-2.48)                            
school_distance  -0.00147**       (-2.69)       1.878***       (4.43)                            
_cons               0.104          (0.60)       117.5          (0.88)                             
----------------------------------------------------------------------
N                   20051                        20051                
----------------------------------------------------------------------
...
For Home A, difference between proposition 13 assessment and actual assessment is \$1,058,824 which is 2 times the purchased price of the home A. For home B the difference is \$1,395,794 which is 2.26 times the purchased price. This difference can be used as an instrumet for property taxes paid. 
...
For Home A, difference between proposition 13 assessment and actual assessment is 3.92 times the purchased price and for home B the difference is  3.56 times the purchased price. This difference can be used as an instrumet for property taxes paid. 
...
The following regression uses homes purchased in 2006 and sold in 2016. The sum of difference between the proposition 13 assessment and acutal assessment normalized by the purchased price is used as the instrument for property taxes paid.
...
controls <- "log(adj_purch_price)+sqft+age+beds+baths+Lot+walk_score+walk_score+avg_school_rating+avg_school_distance|zip_listed_year"
